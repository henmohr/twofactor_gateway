...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const html = `
        <div class="settings-section settings-section--limit-width">
            ...
            <div id="test-message-section" style="margin-top: 30px; padding: 20px; background-color: #f0f7ff; border-radius: 5px; border-left: 4px solid #0085ca;">
                <h3 style="margin-top: 0; color: #0085ca;">Enviar Mensagem de Teste</h3>
                <p style="color: #666;">Envie uma mensagem de teste para validar sua configuração:</p>
                
                <div class="settings-form-field">
                    <label for="test-phone-number">
                        <strong>Número de Telefone para Teste</strong>
                    </label>
                    <p class="field-description">Insira um número de telefone com código do país (Ex: +5511999999999)</p>
                    <input type="text" id="test-phone-number" class="input-field__input" placeholder="+55 11 99999-9999">
                </div>
                
                <div class="settings-form-field">
                    <label for="test-message-text">
                        <strong>Mensagem de Teste (Opcional)</strong>
                    </label>
                    <p class="field-description">Deixe em branco para usar a mensagem padrão</p>
                    <textarea id="test-message-text" class="input-field__input" placeholder="Sua mensagem de teste aqui" style="min-height: 80px;"></textarea>
                </div>
                
                <div class="settings-form-actions">
                    <button type="button" id="send-test-message-btn" class="button secondary">Enviar Mensagem de Teste</button>
                </div>
            </div>
        </div>
    `;
    ...
    if (sendTestMessageBtn) {
        sendTestMessageBtn.addEventListener('click', function(e) {
            e.preventDefault();
            sendTestMessage();
        });
    }
...
function sendTestMessage() {
    const testPhoneNumber = document.getElementById('test-phone-number').value;
    const testMessage = document.getElementById('test-message-text').value || 'Teste de configuração do WhatsApp Cloud API - Sua autenticação em dois fatores está funcionando!';
    
    if (!testPhoneNumber) {
        OC.dialogs.alert('Por favor, insira um número de telefone para teste', 'Erro');
        return;
    }
    ...
    fetch(OC.generateUrl('/ocs/v2.php/apps/twofactor_gateway/api/v1/whatsapp/send-test'), {
        method: 'POST',
        headers: {
            'OCS-APIRequest': 'true',
            'Content-Type': 'application/json'
    ...
    .then(data => {
        ...
        const isOcs = data.ocs ? true : false;
        const meta = isOcs ? data.ocs.meta : { status: data.status || 'ok' };
        const message = isOcs ? data.ocs.meta.message : (data.message || 'Erro ao enviar mensagem');
        
        if (meta.status === 'ok' || (data.ocs && data.ocs.meta.statuscode === 200) || data.statuscode === 200) {
            OC.dialogs.alert('Mensagem de teste enviada com sucesso!', 'Sucesso');
            document.getElementById('test-phone-number').value = '';
            document.getElementById('test-message-text').value = '';
        } else {
            OC.dialogs.alert(message || 'Erro ao enviar mensagem de teste', 'Erro');
        }
    })
    .catch(err => {
        OC.dialogs.alert('Erro: ' + err.message, 'Erro');
